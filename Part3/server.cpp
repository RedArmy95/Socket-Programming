...
//**************Decryption***************
                ...
                const unsigned char * src = (const unsigned char *)temp;
                cerr << endl << strlen((const char*) src) << endl;
                cerr << src << endl;
                ...
                cerr << "dec: " << dec << endl;
                cerr << "len: " << strlen((const char*) dec) << endl;
                RSA_free(payeeRSA);

//***************Decryption********************
                memset(client_message, '\0', sizeof(client_message));

                int money = atoi((const char *)dec);
				for(int i=0 ; i<=MAX_CLIENT ; i++)
				{
					if(i == user_id)
					{
						users[i].acc_balance = users[i].acc_balance + money;		
						break;
					}
				}
				for(int i=0 ; i<=MAX_CLIENT ; i++)
